...
3. ✅ Содержать **один** штамп с актуальными подписантами (Самсонова/Захаров/Березеско/Самсонова, 06.26, <шифр-чертежа>-ОБ-ИОС1.5, Стадия П, Лист 42, Листов 53)
4. ✅ Содержать **только один** экземпляр шифра (Ctrl+F найдёт 1 <шифр-чертежа>, не 4)
...
Тестовый случай: испытуемый PDF <объект-Б> backup.
...
3. **Качество vectorization текста** — TrueType text=Y, но AutoCAD может представить текст как path (если шрифт не embedded и не subsetted). Эмпирически проверить на <шифр-чертежа> markers.
