...
- [[doc-extract]] — извлечение текста/таблиц из PDF (ЕДИНСТВЕННЫЙ вход, маршрутизация по типу листа: pymupdf / MinerU / docling / easyocr).
- [[pdf-edit]] — редактирование PDF (merge/split, страницы, формы, аннотации, вектор-чертежи, визуальный diff). Бывший pdf-helper.
...
- [[id-volume-graph]] — queryable граф устройства тома ИД (модель зависимостей: акты/ведомости/схемы/документы качества/замечания ТН/нормы/принципы) в формате graphify. id-engineer query'ит вместо «простыни». Редактируемый (`id_volume_extract.json` → `rebuild.py`), обезличен. Пока охват ВиВ. **Переехал в блок ПТО:** `blocks/pto/skills/id-volume-graph/` (активация блока — /sync-base).
