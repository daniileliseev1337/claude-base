...
source_file vs source_location (REQUIRED — getting this wrong creates orphan nodes):
- `source_file` is the file the node/edge came from, as a path RELATIVE to the scan root
  (e.g. `agents/auditor.md`, `skills/graphify/SKILL.md`). It is mandatory on every node and
  edge — never null, never an absolute path, never the bare filename.
- `source_location` is ONLY a position inside that file — a line ref like `"L42"` or
  `"L10-L20"` — or `null`. NEVER put a file path in `source_location`. A path in
  `source_location` with `source_file:null` produces an unmergeable orphan node.
