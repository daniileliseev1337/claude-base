...
**[STATUS 2026-05-18 — CLOSED]** Реализовано коммитом `3562631` в
claude-lite-instaler. После Stage 1 (Proxy) Install.ps1 не интерактивно
копирует 5 хелперов в `~/.claude/bin/` и создаёт ярлык в Пуске
«Claude (with proxy)». Verified локально на DANIILPC: 5 файлов
скопированы, ярлык 1905 B создан. CLAUDE.md секция «Прокси» обновлена
с новыми путями.
...
## Дополнение от 2026-07-07 — Урок 17 (stdin хука ↔ OEM-кодировка)

### Симптом
auto-push с 02.07 скипал ВСЕ настоящие SessionEnd как «эфемерные» — 5 коммитов
молча копились локально. auto-pull при этом работал → база выглядела живой,
отставание заметил владелец по `git log origin/main..main`.

### Корень
`[Console]::In.ReadToEnd()` в PS 5.1 без явной InputEncoding читает OEM (cp866).
UTF-8 payload Claude Code с кириллицей в пути (`C:\Users\<Имя>\...jsonl`)
декодировался в мусор (виден в `hook-probe.jsonl`), `Test-Path` не находил
транскрипт, ветка «нет файла = эфемерная» глушила пуш.

### Почему тесты не поймали
SIM-кейсы 02.07 кормили ASCII-пути. Урок: тестовые данные хуков ОБЯЗАНЫ
содержать кириллицу в путях — у команды USERPROFILE кириллический.

### Фикс (коммит 3d3091c)
Перед ReadToEnd — `[Console]::InputEncoding/OutputEncoding = UTF8Encoding($false)`
+ срез BOM (эталон: log-tool-usage.ps1, Блок 2) в auto-push И auto-pull.
Отсутствующий транскрипт теперь fail-open (пуш идёт) — эфемерность доказывается
только subagent-путём или tiny-критерием (строки+длительность).

### Мета
Это ТРЕТЬЯ кодировочная ловушка в хуках за месяц (additionalContext-кракозябры
в log-tool-usage — Блок 2; BOM-parser-error в aggregate — Блок 4; теперь stdin).
Правило: любой новый .ps1-хук, читающий stdin или пишущий в контекст, начинать
с UTF-8-обвязки консоли. Проверка живьём: строка `real SessionEnd (lines=N,
duration_sec=...)` в auto-sync.log на настоящем конце сессии.
