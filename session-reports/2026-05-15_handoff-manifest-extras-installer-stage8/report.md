...
6. Apoliakov-ПК (Поляков Александр Геннадьевич, `Apoliakov@k-7.tech`,
...
Apoliakov-ПК = **Поляков Александр Геннадьевич** (`Apoliakov@k-7.tech`,
hostname `100226745A`), отдельный сотрудник и отдельный хост от
`NB-HP-LQ6G` (Иван Фесенко, `fessenkoim-arch`). В первой версии этого
отчёта я ошибочно атрибутировал инцидент Ивану — увидел Apoliakov в
...
Поляков (на хосте `100226745A`) запустил обновлённый Install.ps1.
Дошёл до **Stage 8**:
...
- **Атрибуция инцидента Stage 8.** В первой версии этого отчёта
  написал «Иван Фесенко (на Apoliakov-ПК)» — это было предположение
  на основании единственной известной мне корп-почты вида `*@k-7.tech`.
  Не проверил GitHub-коммиты автора `Apoliakov@k-7.tech`. Корректировка
  внесена 2026-05-18 после `mcp__fetch__fetch` на GitHub API:
  Apoliakov-ПК = **Поляков Александр Геннадьевич**, отдельный сотрудник,
...
1. **Apoliakov-ПК завершение** — Поляков ещё не подтвердил что
...
- Hostnames: DANIILPC, R-090226727A, DELISEEV-PC, NB-HP-LQ6G,
  100226745A (Apoliakov-ПК)
- GitHub-аккаунты: daniileliseev1337, fessenkoim-arch, Apoliakov
- Email: ifesenko@k-7.tech, Deliseev@k-7.tech, Apoliakov@k-7.tech
...
- **4 ПК** подключены в auto-sync (DANIILPC + R-090226727A +
  NB-HP-LQ6G + 100226745A/Apoliakov)
...
4. **Бонусом нашёл коммит `3b6cd56`** (2026-05-16) от
   `Apoliakov@k-7.tech` — **исправил атрибуцию** Apoliakov-ПК:
   Поляков Александр, не Иван Фесенко. Hostname `100226745A`.
5. **Пользователь подтвердил 7-10 done** (Apoliakov завершение,
