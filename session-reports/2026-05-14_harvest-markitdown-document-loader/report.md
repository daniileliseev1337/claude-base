...
- ✗ Не запушил на `origin/main` — auth: `<логин>` без прав.
...
- Дать ли `<логин>` push-доступ или переключить машину `NB-HP-LQ6G` на отдельный сервисный аккаунт?
...
| Параметр | Значение |
|---|---|
| `HTTP_PROXY` env | `http://Ivan_<сотрудник>:[СЕКРЕТ — не записан]@scuf-meta.ru:10894` |
