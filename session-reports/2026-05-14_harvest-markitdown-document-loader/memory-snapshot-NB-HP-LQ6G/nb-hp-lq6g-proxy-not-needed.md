...
На рабочей машине пользователя `NB-HP-LQ6G` (Windows 11, login `<логин>`) выставлены `HTTP_PROXY` и `HTTPS_PROXY` на корп-прокси `scuf-meta.ru:10894` с inline credentials. При попытке `git push origin main` с активным прокси выдаёт `fatal: ... Proxy CONNECT aborted`. При очистке proxy-env в подпроцессе (`Remove-Item Env:HTTP_PROXY,...`) git ходит до github.com **напрямую** и push работает.
...
См. также [[<логин>-github]], [[git-push-diagnostic-order]].
