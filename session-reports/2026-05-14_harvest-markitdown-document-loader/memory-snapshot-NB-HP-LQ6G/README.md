...
`~/.claude/projects/C--Users-<логин>/memory/` на машине `NB-HP-LQ6G` (Windows 11, login `<логин>`).
...
| `<логин>-github.md` | project | GitHub-аккаунт `<логин>` push'ит в claude-base, collaborator с 2026-05-14 |
...
- Имена аккаунтов: `<логин>`, `<логин>`, `daniileliseev1337`
- Имена репо и пути
...
4. **Решить policy по аккаунтам**: пушим в `claude-base` от **личных** аккаунтов сотрудников (`<логин>`) или с одного **сервисного**? Текущий выбор — личный аккаунт через collaborator-приглашение (по факту работает). Альтернатива — single service account + raspread token через safe channel.
