...
## ФИНАЛ — хвосты закрыты (2026-07-01)
- **§7-guard**: проверен фактом в браузере — клиент-only (c111) шлёт только get_my_client_projects/
  am_i_client/project_requests, БЕЗ projects/clients/transactions. Утренние «лишние запросы» —
  артефакт фоновой employee-сессии в общем network-логе. Правка НЕ нужна, код уже верный.
- **Cleanup тест-данных**: test-employee (e222) снесён полностью; test-client (c111) возвращён в
  исходное беспарольное состояние (has_pw=f, identity удалена); E2E-проекты (91a1, 4f33669a)+3 задачи+
  2 заявки+2 уведомления удалены. Реальные 20 проектов целы.
- **Ветка** feature/customer-2.0 удалена (была слита в main, hash 8e971aa).
- **Git-дрейф устранён**: §7-guard (App.jsx) и миграция 0010 (list_available_executors fallback,
  уже применена к БД) были незакоммичены — деплой/БД опережали git. Закоммичены (fe80599, 8bfb4a2),
  запушены в origin klimat-pro-AI (07f3d33..8bfb4a2 main->main). git синхронизирован, дерево чистое.
- **VPN-правило Direct** (193.124.130.236 + sslip.io) держать в Happ — иначе внешний доступ снова ляжет.

**ИТОГ: «Заказчик 2.0» закрыт полностью.** Фича готова, проверена, прод чист, git синхронизирован,
внешний доступ работает. Открытых задач по теме нет.
