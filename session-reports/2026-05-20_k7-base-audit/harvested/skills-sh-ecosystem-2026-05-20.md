...
| `cad-reader` | ⚠ возможно | К-7 nomenclature внутри; нужна очистка |
| `format` | ❌ нет | Тонкая обёртка над `stroy-formatting`, ничего не добавит |
| `spec-writer` | ❌ нет | Хардкод К-7 объектов («Сит-центр», «Балашиха») |
| `upd-parser` | ❌ нет | RU-specific УПД-формат + adapter из приватного s2-invoice |
| `yandex-disk-uploader` | ❌ нет | К-7 Яндекс.Диск + конкретные subfolder-routes (`02_Договор/`) |

**Итог:** из 12 skills осмысленно публиковать 4-5 (универсальные
поведенческие), 3-4 переписывать (убирать К-7), 3 оставить приватно.
...
наших skills К-7-специфична. Доля кандидатов на public-релиз ~30%
...
«поделиться с командой за пределами К-7». До этого — приватный
