...
(вкладка Code, `CLAUDE_CODE_ENTRYPOINT=claude-desktop`). Машина ПК-хаб, кириллица в путях,
...
## Телефон-доступ — ветка закрыта (2026-06-30)

Пробовали показать карту на телефоне (владелец на работе, ПК дома). Три пути — все
провалились по ОДНОЙ причине: на ПК активен системный VPN-адаптер, рвёт исходящее.
- LAN (`http://<LAN-IP>:8099`) — не годится, телефон не в домашней сети.
- cloudflared quick-tunnel — QUIC к Cloudflare edge таймаутит в цикле (5330 ERR, 0 запросов
  прошло). Подтверждены грабли из контекста «quick-tunnel на ПК-хаб рвётся».
- catbox.moe upload — curl timeout (VPN режет исходящее).
Вывод: пока на ПК системный VPN, туннель/облако-upload С ПК не работают.

РЕШЕНИЕ на будущее (рабочий канал доставки): кидать HTML в СИНКАЕМУЮ ПАПКУ Я.Диска.
У владельца включена синхронизация внешних папок (реестр HKCU\Software\Yandex\Yandex.Disk.2
→ ExternalDirs) — Рабочий стол и рабочие папки на отдельном диске. Файл туда → клиент
YandexDisk2 синкает своим каналом (в обход VPN-egress) → владелец забирает с телефона через
приложение Я.Диск (папка Рабочий стол / Работа). НЕ корень `<USERPROFILE>\YandexDisk\`
— он локально пуст (online-only), сбивает с толку. Не нужны app-password / WebDAV / туннель.
SendUserFile (вложение в чат) у владельца НЕ скачивается — не использовать как канал доставки.
