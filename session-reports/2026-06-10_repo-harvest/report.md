...
| **textsnap** (88⭐, MIT) | CPU-only OCR на PaddleOCR-VL ONNX (~890МБ). РИСК: кириллица не подтверждена, bbox не выдаёт. Пилот = 10 минут: pip install + наш бенч-скан УПД 150dpi; кириллица падает → закрыть |
