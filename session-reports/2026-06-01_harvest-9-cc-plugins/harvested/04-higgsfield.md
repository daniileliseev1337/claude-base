...
## ⚠ Constraint <организация>: подписки не покупаются
Пользователь (2026-06-01): «подписок никаких нет и приобретаться не будут».

### Free tier ПОДТВЕРЖДЁН (скриншот личного кабинета, 2026-06-01)
- **Free Plan** активен.
- **Monthly credits: 10 / 10** — 10 кредитов/мес бесплатно.
- Каждая генерация тратит кредиты (image ~1, video дороже).
- Unlimited models = 0 без подписки.

→ Higgsfield проходит constraint, но **10 credits/мес = небольшой лимит**:
хватит на несколько картинок, на video мало. Для команды — каждый свой
free аккаунт (10/чел).

### Вывод: нужен open-source локальный fallback
Раз 10 credits легко упереть + подписки не покупаются + есть требование
приватности (обезличивание, картинки объектов со шифрами нельзя в облако):

| Вариант | Что | Роль |
|---|---|---|
| **ComfyUI + Flux/SDXL** | Локальная генерация на своём железе | Регулярные картинки, без лимитов, приватные проекты |
| **Higgsfield free** | Облако, 10 credits/мес | Редкие video (Veo/Kling), быстрые пробы |

**Гибрид:** Higgsfield для video и проб (free credits), ComfyUI локально для
регулярных images и приватных проектов. **Приватность критична** — план
объекта со шифром в облако лить нельзя (правило обезличивания CLAUDE.md).
...
5. SMM <организация> — если есть отдельная функция (вопрос пользователю).

### Где Higgsfield дал бы прирост
- High-end cinematic marketing video (15 сек) для соцсетей <организация>.
- Если <организация> делает рекламные ролики проектов — Veo-качество.
...
работает** на их задачах, image/video generation у <организация> **фактически не
...
3. **Credit-based pricing** — кто оплачивает? Корпоративная подписка
   <организация> или личная у каждого?
4. **Auth** — через higgsfield.ai аккаунт. Один корпоративный для команды
   или индивидуальные?

## Идеи в копилку
- ✅ **Veo через Higgsfield** — топ video model 2026.
- ✅ **Soul Character training** — если <организация> имеет mascot/брендинг,
  consistent characters пригодится для marketing материалов.
- ✅ **Virality prediction** — если развиваем SMM <организация>, эта функция оценит
