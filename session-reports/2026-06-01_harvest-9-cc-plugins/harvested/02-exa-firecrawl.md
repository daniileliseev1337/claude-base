...
- ⭐ Stars: **127,004**
- 🍴 Forks: 7,595
- 📜 License: **AGPL-3.0** ✅ для нас (договорённость <организация>, см. `~/.claude/memory/reference_licenses_k7.md`)
- 📅 Created: 2024-04-15
...
#### Firecrawl
- ~~🔴 AGPL-3.0~~ — **снято** (договорённость <организация> + приватность `claude-base`).
- 🟡 API key требуется → управление ключами на 8 ПК = инфраструктура.
  Варианты: (a) один общий ключ команды через `.env` в `claude-base`
  (риск утечки); (b) self-host instance на сервере <организация> (один URL, без ключей).
...
- ✅ Self-host вариант доступен — снимает и cloud dependency, и проблему
  ключей. Требует поднять сервис на инфре <организация>.
...
- AGPL не блокер (договорённости <организация>).
...
1. **Firecrawl deployment:**
   ...
   - (b) Self-host на сервере <организация> — без cloud dependency, без проблем с ключами.
