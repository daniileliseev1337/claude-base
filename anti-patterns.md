...
harvest — анти-паттерн. Часто готовый инструмент (pdfcpu для batch-штампа, и т.п.)
закрывает задачу за минуты.

**Правильно:** **harvest СНАЧАЛА** на задачах, которые пахнут «типовыми» (batch,
конвертация, известный формат) — до того, как писать свой велосипед. См. A10.1.

**Источник:** `2026-05-22_ahp-stamp-overlay` (часы ручных попыток) → `2026-05-25_pdf-stamp-pipeline`
(harvest нашёл pdfcpu, ~1.3 сек/лист). Контраст в одной паре отчётов.
