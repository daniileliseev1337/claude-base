...
формально запрещены принципами. Импорт из аудита <организация> + GH-600 study
...
**Источник:** `image-text-replace/LESSONS-LEARNED.md` §1 (КП <организация> АХП
case, 16 итераций).
...
- <организация> АХП case 2026-05-19: 8 итераций на Arial при scan = Times Bold
...
у <организация> было Times Bold». Это **per-document** характеристика — не
...
Багу не нашли пока не сгенерировали <организация>_audit_report.docx и пользователь
не заметил.
...
R-090226727A — 11 итераций (v6-v11) на 57 листов АХП <объект-Б>,
...
### A3.8 `mcp__word__search_and_replace` дублирует текст в таблицах

На ячейке таблицы с `gridSpan`/vertical-merge/повторяющимися строками сервер считает
ячейку за **N вхождений** (счётчик «occurrences» врёт) и вставляет replace **N раз
подряд в один run** → N-кратное задвоение, документ испорчен. **Акты ИД = одна таблица
целиком** — особо опасно.

❌ Не использовать `search_and_replace` для текста ВНУТРИ таблиц Word.
✅ Точечная in-place правка `word/document.xml` через .NET ZipArchive (`'Update'`):
проверить `count` вхождений в XML **ДО** замены (ждать ровно 1 / известное N; иначе стоп),
целиться в **целый run** (`<w:t>…</w:t>`; cross-run фрагмент `.Replace` не возьмёт),
писать UTF-8 без BOM, бэкап до правки.

**Источник:** feedback `mcp-word-search-replace-duplicates-in-tables` (R-090226727A, 2026-06-05).
Детали + PowerShell-рецепт — [[word-helper]].

### A3.9 Необратимая правка юр. тома ИД по находкам автоприёмки БЕЗ согласования

Приёмка `id-tom-priemka` (роль строгого ТН) флагает «лишнее» — рекламно-каталожные хвосты
заводских инструкций, дубли, спорные листы. Это **КАНДИДАТЫ**, а не команда на удаление.

❌ **Плохо:** приёмка нашла 10 рекламных хвостов → сборщик сам удалил 7 стр из тома
(«привёл в порядок»). Пользователь возмутился: заводские паспорта ТН объекта принимает
**КОМПЛЕКТОМ** (с обложками/контактами), удаление было не нужно и меняло листаж под уже
розданные листы к печати. Откат из бэкапа.

✅ **Хорошо:** находки → в **ведомость замечаний** с разбивкой «кто исправляет». Изъятие/замена
страниц юр. тома ИД — **решение пользователя** (его ответственность перед ТН/заказчиком). Показать
кандидатов + границы, дождаться явного «да». Общее правило harness: необратимое, hard-to-reverse
действие над не-своим артефактом → confirm first (бэкап ≠ индульгенция на самовольство).

**Источник:** сессия Музей Р-1-ВиВ Молот, ось 5 (2026-07-04). Связано: [[karpathy-guidelines]]
(хирургические правки), A1.5 (игнор запрета), skill `id-tom-priemka` (уроки боя 2026-07-04).

### A3.10 Литеральный путь к файлу с «ё»/«й» — NFD-имя из внешнего источника

❌ **Плохо:** Glob/проводник файл видят, но `pdf_info`/`Read`/`open()` по перепечатанному
пути падают «file not found» → агент объявляет файл битым/несуществующим или «перепечатывает
имя внимательнее» (не поможет никогда).

**Причина:** внешние источники (СберБанк-онлайн, macOS-цепочки, некоторые загрузчики) отдают
имена в NFD (`е`+U+0308 вместо `ё` U+0451; `И`+U+0306 вместо `Й`). LLM эмитит текст только
в NFC, NTFS юникод НЕ нормализует → путь честно не существует. Эксперимент 2026-07-11:
pdf-mcp открывает NFC-«ё»/«Ё»/«й» без проблем; NFD-файл по NFC-пути — «not found». Касается
ЛЮБОГО инструмента с литеральным путём, сервер не виноват.

✅ **Хорошо:** сигнатура «not found + Glob видит + в имени ё/й» → wildcard вместо спорной
буквы (`Get-ChildItem -Filter "сч*та.pdf"`, Glob `сч*та`) и работа с копией под ASCII-именем;
программно — `[n for n in os.listdir(d) if unicodedata.normalize("NFC", n) == target]`;
долгосрочно — переименовать файл в NFC-имя.

**Источник:** сессия 2026-07-11 (реквизиты Сбера «…счёта.pdf»), воспроизведение парами
NFC/NFD тест-PDF. Связано: A7.4 (cv2 и кириллица в путях), skill `doc-extract` (ловушки).
...
**Источник:** аудит <организация> + наш CLAUDE.md (раздел Безопасность).
...
перед обработкой, либо проверить атрибут ReparsePoint. Уточнение (2026-06-05): при
переименовании через PowerShell COM `OrderedDictionary[int]` — доступ по ПОЗИЦИИ, не по
ключу-числу (легко перепутать индекс с именем).

**Источник:** `2026-06-03_vso-reformat`, `2026-06-05_id-study-method-and-yandex-rename` (R-090226727A).
...
`cloudflared.exe` 63 MB в <организация> базе — анти-паттерн. Должно быть
...
**Источник:** аудит <организация>.

### A10.3 Дубль скилла (fork анти-паттерн)

В <организация> был `pdf-advanced` рядом с `pdf` (copy Anthropic). Это анти-паттерн.
...
## Категория 11. Визуальная сверка и computer-use

### A11.1 Всматривание в низкоразрешённый растр вместо источника высокого разрешения

Когда надо **прочитать/сверить мелкую деталь** (текст, размер, номер, символ) на изображении:

❌ **Плохо:** взять низкоразрешённый растр (полный скрин экрана, мелкий PNG-экспорт
страницы, ужатый скан) и цифрово его увеличить (computer-use `zoom`, кроп+upscale,
«приблизь скрин/картинку»). Пикселей больше не станет — та же каша → «нечитабельно»
→ ошибки и выдумки.

✅ **Правильно — поднять разрешение У ИСТОЧНИКА, не у готового кадра:**
- **Растр из PDF** (`*_pN.png` — экспорт страницы): вернуться к исходному PDF → текст-слой
  через pdf-mcp (`pdf_search` / `pdf_read_pages`) или `pdf_render_pages` в высоком DPI.
  НЕ читать мелкий PNG.
- **Живое ПО (AutoCAD/Revit/CAD):** зум колесом/окном к области прямо в DWG → потом скрин.
- **Браузер (claude-in-chrome):** Ctrl-«+» / zoom, либо читать DOM-текст (`get_page_text`).
- **Скан/фото без текст-слоя:** OCR (skill [[image-text-replace]]) либо кроп области из
  **максимального** разрешения оригинала — не из уменьшенной копии.
- **Любое GUI:** собственный zoom; крайний случай — экранная лупа ОС (Win «+»).

**Почему:** разрешение добавляется только при ре-рендере из источника (вектор / текст-слой /
высокий DPI); апскейл готового растра информации не добавляет. 2 действия дешевле ошибки.

**Форсится хуком:** `PreToolUse` matcher `screenshot|zoom` → `scripts/screenshot-source-zoom.ps1`
инжектит это напоминание при каждом вызове скрин-инструмента (гарантия на механизме, не на тексте).

**Источник:** наблюдение владельца 2026-07-04 — повторяющийся паттерн. Живой пример `pass_p4.png`
(паспорт ZPVR): разворот ужат в мелкий PNG, текст = сплошные пиксели; правильно — текст-слой
исходного PDF через pdf-mcp.

---

## Категория 11. Проектирование AI-конвейеров

### A11.1 AI напрямую водит целевой API без инспектируемого артефакта

❌ **Плохо:** модель напрямую дёргает API целевой системы (Revit, БД, CAD,
платёжка) — чёрный ящик: нет точки проверки ДО необратимых действий, результат
нельзя отревьюить, переиграть или исполнить без модели.

✅ **Хорошо:** между моделью и исполнителем — **инспектируемый промежуточный
артефакт** (JSON по документированной схеме / скрипт / промпт с эталонами):
`AI → артефакт → валидация кодом → превью/ревью человеком → детерминированный
executor`. Executor всеяден — не знает, кто написал артефакт (LLM, человек,
другой скрипт).

**Источник:** разбор FamFab (AI-генератор семейств Revit, MIT) 2026-07-02 —
его 3-стадийная архитектура именно такова; + продакшен-кейс гибридного
конвейера. Методичка: `memory/reference_hybrid_ai_pipeline.md`.

### A11.2 Молчаливый 0 / дефолт вместо ошибки — тихая порча данных

❌ **Плохо:** `catch { return 0; }` в вычислении выражения: опечатка в имени
параметра → геометрия/сумма молча неверна. Тихая порча данных ХУЖЕ падения —
падение видно, порча уезжает заказчику.

✅ **Хорошо:** fail fast (исключение с именем ненайденного параметра) либо
явный маркер ошибки + строка в лог. Молча подставлять дефолт запрещено везде,
где результат уходит в данные/геометрию/деньги. «Невозможное» значение
(0, пусто) в результате — симптом, проверять как ошибку.

**Источник:** FamFab executor `EvaluateExpression` (C# тихо возвращает 0,
JS-парсер того же проекта честно кидает Unknown parameter — контраст в одном
репо). Родственник по духу: ловушка «.xlsx None ≠ пусто» (CLAUDE.md).
...
- 2026-05-20 — первая версия. Собрано из 6+ источников
  ...
  hooks-debugging memory, session-report-policy memory, аудит <организация>,
  GH-600 study guide).
- Обновлять при появлении нового anti-pattern в session-report или
  feedback-memory.
- 2026-07-02 — Категория 11 «Проектирование AI-конвейеров» (A11.1 инспектируемый
  артефакт, A11.2 молчаливый 0) — из разбора FamFab + паттерна гибридного конвейера.
